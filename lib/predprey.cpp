...
int initCommunicationSensor(struct robot *cro);
void updateCommunicationSensor(struct robot *cro, int noutputs);
/*
 * env constructor
 */
Problem::Problem()
...
//       ninputs += initBiasSensor(ro);
       ninputs += initCommunicationSensor(ro);
       ...
       noutputs = ro->motorwheels + ro->motorleds + 1;
...
void Problem::reset()
{
    ...
    for (r=0, ro=rob; r < nrobots; r++, ro++)
    {
        // TODO: check if edges are added
        ro->goal[0] = rng->getInt(0, 1);
        ro->goal[1] = rng->getInt(0, 1);
        ro->goal[2] = rng->getInt(0, 3);

        ro->color = ro->goal[0] * 8 + ro->goal[1] * 4 + ro->goal[2];
...
//        updateBiasSensor(ro);
        updateCommunicationSensor(ro, noutputs);
...
double Problem::step()
{
    ...
    *cdone = 0;
    ...
    wx = worldx / 2.0;
    wy = worldy / 2.0;
    double maxdist = sqrt((wx*wx)+(wy*wy));
...
/*
* initialize the communication sensor and return the number of sensory neurons to be created
*/
int initCommunicationSensor(struct robot *cro)
{
    return(1);
...
{
    if (cro->idn == 0)
        *cro->csensors = caction[noutputs + 2];
    else
        *cro->csensors = caction[2];
    cro->csensors++;
}
